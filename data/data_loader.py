...
class NumpyTupleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, (slice, list, np.ndarray)):
            ...
            batches = [tuple([batch[i] for batch in batches]) for i in range(length)]  # six.moves.range(length)]
        else:
    ...
    def save(cls, filepath, numpy_tuple_dataset):
        ...
        if not isinstance(numpy_tuple_dataset, NumpyTupleDataset):
            raise TypeError(
                "numpy_tuple_dataset is not instance of " "NumpyTupleDataset, got {}".format(type(numpy_tuple_dataset))
            )
