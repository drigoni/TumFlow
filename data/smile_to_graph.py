...
class GGNNPreprocessor(object):
    ...
    def __init__(self, max_atoms=-1, out_size=-1, add_Hs=False, kekulize=False):
        ...
        if max_atoms >= 0 and out_size >= 0 and max_atoms > out_size:
            raise ValueError("max_atoms {} must be less or equal to " "out_size {}".format(max_atoms, out_size))
...
def type_check_num_atoms(mol, num_max_atoms=-1):
    ...
    if num_max_atoms >= 0 and num_atoms > num_max_atoms:
        ...
        raise MolFeatureExtractionError(
            "Number of atoms in mol {} exceeds num_max_atoms {}".format(num_atoms, num_max_atoms)
        )
...
def construct_adj_matrix(mol, out_size=-1, self_connection=True):
    ...
    if s0 != s1:
        raise ValueError(
            "The adjacent matrix of the input molecule" "has an invalid shape: ({}, {}). " "It must be square.".format(
                s0, s1
            )
        )
...
def construct_discrete_edge_matrix(mol, out_size=-1):
    ...
    if out_size < 0:
        ...
        raise ValueError("out_size {} is smaller than number of atoms in mol {}".format(out_size, N))
