...
def generate_mols(model, temp=0.7, z_mu=None, batch_size=20, true_adj=None, device=-1):
...
def generate_mols_interpolation_grid(model, z0=None, true_adj=None, seed=0, mols_per_row=13, device=None, delta=1.0):
    np.random.seed(seed)
...
def visualize_interpolation(
    ...
    if mol_smiles is not None:
        ...
        with torch.no_grad():
            ...
            z0 = torch.cat((z0[0], z0[1]), dim=1).squeeze(dim=0)
            z0 = _to_numpy_array(z0)

    adjm, xm = generate_mols_interpolation_grid(
        model, z0=z0, mols_per_row=mols_per_row, delta=delta, seed=seed, device=device
    )
    ...
    if correct:
        ...
        interpolation_mols = [
            valid_mol(construct_mol(x_elem, adj_elem, atomic_num_list)) for x_elem, adj_elem in zip(xm, adjm)
        ]
    ...
    print(
        "len(interpolation_mols):{}, len(valid_mols):{}, len(valid_mols_smiles_unique):{}".format(
    ...
    img = Draw.MolsToGridImage(
        valid_mols_unique[:k], molsPerRow=molsPerRow, legends=valid_mols_smiles_unique_label[:k], subImgSize=psize
    )
...
def visualize_interpolation_between_2_points(
    ...
    if mol_smiles is not None:
        ...
        with torch.no_grad():
            ...
            z1 = torch.cat((z1[0], z1[1]), dim=1).squeeze(dim=0)
    ...
    psize = (200, 200)
    img = Draw.MolsToGridImage(
        valid_mols_unique, legends=valid_mols_smiles_unique_label, molsPerRow=mols_per_row, subImgSize=psize
    )
    img.save(filepath + "_.png")
...
def plot_colormap():
    ...
    a = [
        [0.42, 0.41, 0.38, 0.32, 0.44, 0.37, 0.37, 0.37, 0.31],
        [0.53, 0.53, 0.53, 0.68, 0.68, 0.57, 0.57, 0.43, 0.32],
        [1, 0.52, 0.68, 0.68, 0.68, 0.68, 0.57, 0.47, 0.39],
        [1, 1, 1, 0.68, 0.68, 0.68, 1, 1, 0.57],
        [1, 1, 1, 1, 1, 1, 1, 1, 0.68],
        [0.59, 0.59, 0.59, 0.59, 0.59, 0.59, 0.79, 0.79, 0.53],
        [0.59, 0.59, 0.59, 0.59, 0.59, 0.59, 0.59, 0.53, 0.71],
        [0.34, 0.59, 0.59, 0.59, 0.59, 0.59, 0.59, 0.54, 0.71],
        [0.32, 0.34, 0.59, 0.59, 0.59, 0.59, 0.32, 0.54, 0.54],
    ]

    am = np.array(a)
    fig, ax = plt.subplots(figsize=(12, 12))
    ax = sns.heatmap(am, annot=True, fmt=".2f", linewidths=0.5, ax=ax, annot_kws={"size": 18}, vmin=0, vmax=1)
    plt.show()
...
def main():
    ...
    parser.add_argument("--hyperparams-path", type=str, default="tumflow-params.json", required=True)
    ...
    parser.add_argument(
        "--correct_validity", type=strtobool, default="true", help="if apply validity correction after the generation"
    )
    ...
    print(
        "{} in total, {}  training data, {}  testing data, {} batchsize, train/batchsize {}".format(
            len(dataset), len(train), len(test), batch_size, len(train) / batch_size
        )
    )
    # 1. Reconstruction
    if args.reconstruct:
    ...
    for i in range(args.n_experiments):
        # 1. Random generation
        adj, x = generate_mols(model, batch_size=batch_size, true_adj=None, temp=args.temperature, device=device)
        ...
        if args.save_score:
            ...
            smiles_qed_plogp = [
                (sm, env.qed(mol), env.penalized_logp(mol))
        ...
        if save_fig:
            ...
            img = Draw.MolsToGridImage(
                val_res["valid_mols"], legends=val_res["valid_smiles"], molsPerRow=20, subImgSize=(300, 300)
            )  # , useSVG=True
            img.save(filepath)
