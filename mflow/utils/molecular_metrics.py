...
class MolecularMetrics(object):
    ...
    def natural_product_scores(mols, norm=False):
        ...
        scores = [
            sum(NP_model.get(bit, 0) for bit in Chem.rdMolDescriptors.GetMorganFingerprint(mol, 2).GetNonzeroElements())
    ...
    def water_octanol_partition_coefficient_scores(mols, norm=False):
        scores = [
            MolecularMetrics._avoid_sanitization_error(lambda: Crippen.MolLogP(Chem.AddHs(mol, 1)))
            if mol is not None
            else None
            for mol in mols
        ]
    ...
    def drugcandidate_scores(mols, data):
        scores = (
            MolecularMetrics.constant_bump(
                MolecularMetrics.water_octanol_partition_coefficient_scores(mols, norm=True), 0.210, 0.945
            )
    ...
    def constant_bump(x, x_low, x_high, decay=0.025):
        return np.select(
            ...
            choicelist=[np.exp(-((x - x_low) ** 2) / decay), np.exp(-((x - x_high) ** 2) / decay)],
